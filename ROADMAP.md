...
### [0.5.2] - 2026-02-26 · 深层安全架构修复（当前版本）✅

- [x] **生产环境物理封闭 Swagger UI**：防范 OpenAPI 信息侧漏风险，生产环境自动拔除 `/docs` 相关敏感路由。
- [x] **根除测试框架对凭据的暴力覆写**：重构测试体系为 "Configuration-Driven"（配置优先驱动），防止假阳性绿色安全通过错觉。
- [x] 新增生产环境 404 反向探测防御测试用例。

### [0.5.1] - 2026-02-25 · 安全热修复与文档重构 ✅

- [x] **强制阻塞无凭证渠道注册**：取消开发环境兼容设定，全级别强制 `ENGRAMA_ADMIN_TOKEN` 拦截。
...
| 版本 | 状态 | 当前能力 |
|---|---|---|
| **v0.5.2** | ✅ 当前版本 | 完整的存-取-搜链路 + 生产级基础设施 + MCP + 鉴权体系 + 严格的暴露面封堵 |
